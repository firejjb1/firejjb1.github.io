...
4. You should now be able to capture frames not in the default rendering backend: Ganesh GL (chrome://gpu for checking which rendering backend is running). However, if you want to try the newest and latest Skia Graphite backend, there's some extra work to do.
5. For graphite, if --in-process-gpu is set, graphite will automatically be disabled. The logic for this is in gpu/ipc/service/gpu_init.cc. Manually remove this logic and rebuild. An LLM can help figure this out if you're stuck.
